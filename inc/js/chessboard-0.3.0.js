...
//------------------------------------------------------------------------------
    var COLUMNS = 'abcdefghijklmnopqrstu'.split('');
    var SIZE = 16;
    ...
    function validSquare(square) {
        if (typeof square !== 'string')
            return false;
        return (square.search(/^[a-z][0-9][0-9]?$/) !== -1);
    }

    function validPieceCode(code) {
        if (typeof code !== 'string')
            return false;
        return (code.search(/^[bw][AKQRNBP]$/) !== -1);
    }
    ...
    function validFen(fen) {
        ...
        for (var i = 0; i < SIZE; i++) {
            if (chunks[i] === '' ||
                    ...
                    chunks[i].search(/[^akqrbnpAKQRNBP1-8]/) !== -1) {
                return false;
            }
        }
...
// returns false if the obj is not a valid position object
    function objToFen(obj) {
        ...
        fen = fen.replace(/1111111111111111111/g, '19');
        fen = fen.replace(/111111111111111111/g, '18');
        fen = fen.replace(/11111111111111111/g, '17');
        fen = fen.replace(/1111111111111111/g, '16');
        fen = fen.replace(/111111111111111/g, '15');
        fen = fen.replace(/11111111111111/g, '14');
        fen = fen.replace(/1111111111111/g, '13');
        fen = fen.replace(/111111111111/g, '12');
        fen = fen.replace(/11111111111/g, '11');
...
//------------------------------------------------------------------------------
        ...
        function buildSparePieces(color) {
            var pieces = ['wA','wK', 'wQ', 'wR', 'wB', 'wN', 'wP'];
            if (color === 'black') {
                pieces = ['bA','bK', 'bQ', 'bR', 'bB', 'bN', 'bP'];
            }
